...
export function GroupInfoPanel({ group, onClose, onChatCleared }) {
  ...
  const isAdmin = currentUser?.role === "GLOBAL_ADMIN" || currentUser?.role === "BACK_OFFICE_MANAGER";

  const fetchMembers = async (currentPage, currentSearch, append = false) => {
    setIsLoading(true);
    try {
      const res = await api.get(`/groups/${group.id}/members`, {
        params: { page: currentPage, limit: MEMBERS_PER_PAGE, search: currentSearch }
      });
  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-slate-200 overflow-hidden w-80 shrink-0 shadow-sm">
      <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
        <div>
          <h2 className="text-base font-bold text-slate-900 truncate pr-2">{group.name}</h2>
          <p className="text-xs text-slate-500">Group Information</p>
        </div>
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600 xl:hidden">
          <X size={20} />
        </button>
      </div>
      ...
      {isAdmin && (
        <div className="p-4 border-b border-slate-100 bg-red-50/50">
          ...
          <Button 
            variant="outline" size="sm" onClick={handleClearChat} disabled={isClearingChat}
      ...
      <div className="p-4 border-b border-slate-100">
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Members</p>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {members.length === 0 && !isLoading ? (
          ...
          <ul className="space-y-1">
            {members.map(member => (
              <li key={member.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100">
        ...
        {hasMore && (
          <div className="p-2 text-center mt-2 border-t border-slate-50">
            <Button 
              type="button" variant="secondary" size="sm" onClick={handleLoadMore} disabled={isLoading}
