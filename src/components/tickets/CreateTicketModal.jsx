...
//                   className={`flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[14px] font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all ${isBackOfficeStaff
...
//                     className={`flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[14px] font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${user?.role === "BACK_OFFICE_MEMBER"
//                         ? "pointer-events-none opacity-70"
//                         : "hover:bg-white"
//                       }`}
//                   >
...
import toast from "react-hot-toast";
import { X, Loader2, Store, Users, Calendar, AlertCircle } from "lucide-react";
import axios from "axios"; // 🟢 Added axios for direct S3 uploads
...
export const CreateTicketModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  // 🟢 NEW: Added upload progress state
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (file) {
        // 🟢 1. Check file size first (25MB Limit)
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-6 pb-2">
              ...
              <Button
                ...
                className="rounded-xl px-8 h-11 bg-[#0F172A] hover:bg-indigo-600 text-white font-bold transition-all shadow-sm active:scale-95 disabled:opacity-50"
              >
                {/* 🟢 NEW: Enhanced button text showing upload progress */}
                {isUploading ? (
                  <span className="flex items-center gap-2">
